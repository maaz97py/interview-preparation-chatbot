...
class Moderator:
    ...
    def __call__(self, input_object: str) -> str:
        """
        Invokes the Moderator based on the input object.

        Parameters
        ----------
        input_object : str
            The input object, which could be either text or an audio file.

        Returns
        -------
        str
            The recognized text if input is audio, or the filename of the temporary WAV file if input is text.
        """
        if ".wav" in input_object:
            # If the input is an audio file, convert speech to text
            return self.speech_to_text(input_object)
        else:
            # If the input is text, convert text to speech and return the filename of the temporary WAV file
            return self.text_to_speech(input_object)
